...
import com.moko.lw007.databinding.Lw007ActivityConnSettingNewBinding;
...
public class LoRaConnSettingNewActivity extends BaseActivity implements CompoundButton.OnCheckedChangeListener {
    private Lw007ActivityConnSettingNewBinding mBind;
    ...
    private String[] mSingleChannelArray = {"868.1 MHZ", "868.3 MHZ", "868.5 MHZ"};

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        mBind = Lw007ActivityConnSettingNewBinding.inflate(getLayoutInflater());
        ...
        if (!LoRaLW007MokoSupport.getInstance().isBluetoothOpen()) {
            ...
            orderTasks.add(OrderTaskAssembler.getLoraSingleChannelFunction());
            orderTasks.add(OrderTaskAssembler.getLoraSingleChannelSelection());
                        ...
                        if (value.length >= 4) {
                            ...
                            if (flag == 0x01) {
                                ...
                                switch (configKeyEnum) {
                                    ...
                                    case KEY_LORA_SINGLE_CHANNEL_FUNCTION:
                                    case KEY_LORA_SINGLE_CHANNEL_SELECTION:
                                        savedParamsError |= result != 1;
                                        break;
                                    case KEY_LORA_UPLINK_STRATEGY:
                                        savedParamsError |= result != 1;
                                        if (savedParamsError) {
                                            ToastUtils.showToast(LoRaConnSettingNewActivity.this, "Opps！Save failed. Please check the input characters and try again.");
                                        } else {
                                            showSyncingProgressDialog();
                                ...
                                switch (configKeyEnum) {
                                    ...
                                    case KEY_LORA_REGION:
                                        if (length > 0) {
                                            ...
                                            initCHDRRange();
                                            initDutyCycle();
                                            mBind.llSingleChannel.setVisibility(mSelectedRegion == 5 ? View.VISIBLE : View.GONE);
                                        }
                                        break;
                                    case KEY_LORA_MESSAGE_TYPE:
                                        if (length > 0) {
                                    ...
                                    case KEY_LORA_SINGLE_CHANNEL_FUNCTION:
                                        if (length > 0) {
                                            final int dutyCycleEnable = value[4] & 0xFF;
                                            mBind.cbSingleChannelEnable.setChecked(dutyCycleEnable == 1);
                                        }
                                        break;
                                    case KEY_LORA_SINGLE_CHANNEL_SELECTION:
                                        if (length > 0) {
                                            final int selected = value[4] & 0xFF;
                                            mBind.tvSingleChannelSelection.setTag(selected);
                                            mBind.tvSingleChannelSelection.setText(mSingleChannelArray[selected]);
                                        }
                                        break;
                                }
                            }
    ...
    public void selectServerRegion(View view) {
        ...
        bottomDialog.setListener(value -> {
            if (mSelectedServerRegion != value) {
                ...
                updateCHDR();
                initDutyCycle();
                mBind.llSingleChannel.setVisibility(mSelectedServerRegion == 1 ? View.VISIBLE : View.GONE);
            }
        });
        bottomDialog.show(getSupportFragmentManager());
    }
    ...
    public void onSelectSingleChannel(View view) {
        if (isWindowLocked())
            return;
        int selected = (int) view.getTag();
        BottomDialog bottomDialog = new BottomDialog();
        bottomDialog.setDatas(new ArrayList<>(Arrays.asList(mSingleChannelArray)), selected);
        bottomDialog.setListener(value -> {
            mBind.tvSingleChannelSelection.setTag(value);
            mBind.tvSingleChannelSelection.setText(mSingleChannelArray[value]);
        });
        bottomDialog.show(getSupportFragmentManager());
    }
    ...
    public void onSave(View view) {
        ...
        showSyncingProgressDialog();
        LoRaLW007MokoSupport.getInstance().sendOrder(mOrderTasks.toArray(new OrderTask[]{}));
    }
    ...
    private ArrayList<OrderTask> getOrderTasks() {
        ...
        if (mSelectedPlatform == 0) {
            ...
            if (mSelectedRegion == 3 || mSelectedRegion == 4
                ...
                if (mSelectedRegion == 5) {
                    orderTasks.add(OrderTaskAssembler.setLoraSingleChannelFunction(mBind.cbSingleChannelEnable.isChecked() ? 1 : 0));
                    int selected = (int) mBind.tvSingleChannelSelection.getTag();
                    orderTasks.add(OrderTaskAssembler.setLoraSingleChannelSelection(selected));
                }
            }
    ...
    private static final String DEVICE_PROFILE_TYPE = "007_PIR";
    private static final String APPLICATION_NAME = "LW007_PIR";
    // LW001-BG PRO(L) 10
    // LW001-BG PRO(M) 20
    // LW004-PB 30
    // LW005-MP 40
    ...
    // LW007-PIR 50
    // LW008-MT 60
    private static final String PRODUCT_MODEL = "50";
                ...
                .execute(new StringCallback() {
                    ...
                    public void onSuccess(Response<String> response) {
                        ...
                        if (commonResp.code != 200) {
                            ToastUtils.showToast(LoRaConnSettingNewActivity.this, commonResp.msg);
                            LoginDialog dialog = new LoginDialog();
                        ...
                        mAccount = account;
                        IoTDMSPUtils.setStringValue(LoRaConnSettingNewActivity.this, IoTDMConstants.EXTRA_KEY_LOGIN_ACCOUNT, account);
                        IoTDMSPUtils.setStringValue(LoRaConnSettingNewActivity.this, IoTDMConstants.EXTRA_KEY_LOGIN_PASSWORD, password);
                        IoTDMSPUtils.setIntValue(LoRaConnSettingNewActivity.this, IoTDMConstants.EXTRA_KEY_LOGIN_ENV, envValue);
                        // add header
                    ...
                    public void onError(Response<String> response) {
                        ToastUtils.showToast(LoRaConnSettingNewActivity.this, R.string.request_error);
                        LoginDialog dialog = new LoginDialog();
        ...
        OkGo.<String>post(Urls.syncGatewayApi(getApplicationContext()))
                ...
                .execute(new StringCallback() {
                    ...
                    public void onSuccess(Response<String> response) {
                        ...
                        if (code != 200) {
                            ToastUtils.showToast(LoRaConnSettingNewActivity.this, msg);
                            return;
                        }
